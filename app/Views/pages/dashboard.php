...
<div class="mt-6" x-data="modal">
  ...
  <div class="fixed inset-0 backdrop-blur-sm backdrop-brightness-50 z-[1]" x-show="show" 
    ...
    x-cloak
  ></div>
  <div class=" text-text-dark  z-10 fixed inset-0 grid place-items-center  " x-show="show" x-transition x-cloak>
    <div class="w-11/12 max-w-2xl p-6 pt-8 rounded-lg bg-white -mt-10 bg-gray-white text-black dark:text-white dark:bg-[#1A1B1E]" @click.outside="hide">
      ...
      <form action="/" method="post" class="mt-10">
        <div class="mb-2">
          ...
          <input type="text" class="w-full rounded-md bg-white dark:bg-secondary-dark text-sm p-2 px-5 border border-gray-300 dark:border-gray-700 outline-none focus:border-blue-500 dark:focus:border-blue-500 " name="title" id="title" required>
        </div>
        <div class="">
          <label for="body" class="text-sm">Body*</label>
          <textarea name="body" id="body" class="w-full rounded-md resize-none bg-white dark:bg-secondary-dark text-sm p-2 px-5 border border-gray-300 dark:border-gray-700 outline-none focus:border-blue-500 dark:focus:border-blue-500 " rows="10" required></textarea>
        </div>
        ...
        <div class="bg-white dark:bg-secondary-dark p-6 flex flex-col justify-between rounded-md min-h-[15rem]">
          ...
          <div class="mt-6 flex gap-2">
            ...
            <form action="archive/<?= $d["note_id"] ?> " method="post"  class="relative">
              ...
              <span class="bg-black w-max py-1 px-2 rounded-md text-sm text-text-dark invisible transition duration-300 opacity-0 absolute  peer-hover:visible peer-hover:opacity-100 -top-10 left-1/2 -translate-x-1/2 ">Archive Note</span>
            </form>
            <a href="detail/<?= $d["note_id"] ?> "  class="relative">
